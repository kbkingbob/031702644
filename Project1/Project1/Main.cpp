...
/*void todo0(wstring op,wstring add,wstring phon,wstring name,bool fl)
{
	...
	for (i = pos; i < add.length(); i++)
	{
		if (add[i] == L'镇' || add[i] == L'乡' || add[i] == L'道' || add[i] == L'街' || add[i] == L'村')
		{
	...
	wcout << L"{" << L'"' << L"姓名" << L'"' << L':' << L'"' << name << L'"' << L',' << L'"' << L"手机" << L'"' << L':' << L'"' << phon << L'"' << L',' << L'"' << L"地址" << L'"' << L":[";
	if (op == L"1")
	{
		for (i = 0; i < 5; i++)
		{
...
//			wcout.imbue(locale("chs"));
	...
	//		freopen_s(&stream1, "2.txt", "w", stdout);
		...
		//wcout << L'"' << L"姓名" << L':' << L'"' << name << L'"' << L',' << L'"' << L"手机" << L'"' << L':' << L'"' << phon << L'"' << L',' << L'"' << L"地址" << ":[";
	//	wcout << ;
		//solve ans = solve(op, add, phon, name,bl);
		//ans.todo(answ);
	//	wcout << answ;
		//todo(op, add, phon, name, bl);
